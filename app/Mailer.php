...
class Mailer
{
	...
	public static $retriableErrors = [
		'Quota exceeded' => ['quota'],
		'FROM problem' => ['The following From address failed', 'MAIL FROM command failed,Try again later'],
		'Temporary' => ['Temporary'],
	...
	public static function sendByRowQueue($rowQueue, $ignoreStatus = false)
	{
		...
		try {
			...
			if ($status) {
				...
				// if error is quota 
				if (count(array_filter(self::$retriableErrors, function ($patterns) use ($error) {
					$result = true;
