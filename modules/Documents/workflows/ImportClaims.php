...
class analizator_claim
{
    ...
    function wykryj(string $k, array &$nag, ?string $xv): ?array
    {
        ...
        if ($rt = self::testfield($k, $nag, 'insuredname', 'insured_name', $xv)) {
            ...
            return $rt;
        } elseif ($rt = self::testfield($k, $nag, 'hoattorneyfirm', 'ho_law_firm', $xv)) {
            return $rt;
        } elseif ($rt = self::testfield($k, $nag, 'hopafirm', 'ho_pa_firm', $xv)) {
            return $rt;
            ...
            return $rt;
        } elseif ($rt = self::testfield($k, $nag, 'mortgagecompany', 'mortgage_company', $xv)) {
            return $rt;
        } elseif ($rt = self::testfield($k, $nag, 'mortgageloan#', 'mortgage_loan_number', $xv)) {
            return $rt;
...
class ImportClaims
{
    ...
    public static function import_claims_from_excel(Documents_Record_Model $recordModel)
    {
        ...
        try {
            ...
            if ($nums[0] > 0) {
                ...
                $workflow = $wfs->retrieveByName('CREATE_PUBLIC_ADJUSTER_CONFIRMATION_REQUESTS', 'Portfolios');
            }
            try {
                if (isset($workflow))
    ...
    public function utworz_nowe_claimy_w_pam()
    {
        ...
        foreach ($this->xlClaims as $xc) {
            ...
            if (!isset($c)) {
                if (!empty($xc->ho_law_firm)) {
                    if ($isPA) {
                        $xc->status = claim_in_xls_row::BLAD_REK;
                        $xc->status_row = "PA and HO claims in the same file";
                        goto next_xc;
                    }
